...
let login = async (req, res) => {
    try {
        ...
        if (!logonRegistered_user) {
            return res.status(401).json({ message: "Invalid email or password" });  // 401 for unauthorized access
        }
...
// lec 31 // to send user data to the frontend
...
let user = async (req, res) => {
    try {
       ...
       res.status(200).json({msg : userData});
        ...
        console.log("Controller side error in controller in user", error);
    }
}


module.exports = { Home, register, login, contact, logout, user };